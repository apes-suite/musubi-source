...
program mus_cm_to_pdf_test
  use env_module,          only: rk, eps
  use tem_general_module,  only: tem_start, tem_finalize, tem_general_type
  use mus_compute_cumulant_module, only: central_moment_split, cm_to_pdf
  ...
  integer :: ii, jj, kk
  real(kind=rk) ::  f(-1:1,-1:1,-1:1) = -100.0_rk
  real(kind=rk) :: ff(-1:1,-1:1,-1:1) = -234.0_rk
  real(kind=rk) :: k(0:2,0:2,0:2) = -123.0_rk
  ...
  ff = cm_to_pdf( k, ux, uy, uz )

        write(*,"(3A3,2A15  )") 'ii','jj','kk','f','ff'
  do kk = -1, 1
    do jj = -1, 1
      do ii = -1, 1
        write(*,"(3I3,2F15.11)") ii,jj,kk, f(ii,jj,kk), ff(ii,jj,kk)
      end do
    end do
  end do

  err_sum = sum( abs(f-ff) )
  ...
  if (error) then
    write(*,"(A,F18.14)") 'tolerance: ', tolerance
    write(*,"(A,F18.14)") '    error: ', err_sum
    write(*,"(A)") 'FAILED'
  else
    write(*,'(A)') 'PASSED'
  end if
...
end program mus_cm_to_pdf_test
