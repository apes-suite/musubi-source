...
use mus_particle_perfectcube_module,   only : perfectcube_params, &
                                            & createParticles_perfectcube, &
                                            & createParticles_onTreeIDs
...
!> Initialization for particleGroup and all the particles in it for perfect cube scaling tests
!! Includes:
!! * Assigning the required procedure pointers for particleGroup and particles
!! * Particle communication routines
!! * Building the representation of the particles on the grid
...
!! * Particle communication routines
!! * Building the representation of the particles on the grid
subroutine mus_particles_initialize_perfectcube( particleGroup, scheme, &
                                   & geometry, params       )
  !> Array of particles
  type(mus_particle_group_type), target :: particleGroup
  !> Scheme for access to leveldescriptor
  ...
  type(mus_param_type), intent(in) :: params
  !---------------------------------------------------!
  integer :: lev, maxParticles, nBCs, BCid, nProcs
  integer :: i
  integer :: debugLogUnit
  integer :: iParticle, iproc, iBC
  logical :: rmflag       ! .TRUE. if particle should be removed from group
  ...
  integer :: logUnit
  ! For perfect cube scaling test only
  ...
  ! For perfect cube scaling test: fill the particleGroup with particles
  call getProcessBoundingBox( scheme = scheme, geometry = geometry, &
                            & boundingBox = processBoundingBox      )

  boundingBoxOrigin(1) = processBoundingBox(1)
  boundingBoxOrigin(2) = processBoundingBox(3)
  boundingBoxOrigin(3) = processBoundingBox(5)
  boundingBoxLength = processBoundingBox(2) - processBoundingBox(1)

  ! write(stdOutUnit,*) "ProcessBoundingBox = ", processBoundingBox
  call init_da_particle_DPS(particleGroup%particles_DPS, particleGroup%nParticles)


  call createParticles_onTreeIDs(                                         &
              & particleGroup = particleGroup,                            &
              & scheme        = scheme,                                   &
              & geometry      = geometry,                                 &
              & params        = params,                                   &
              & radius        = perfectcube_params%radius,                &
              & mass          = perfectcube_params%mass,                  &
              & Nparticles    = perfectcube_params%particles_per_process, &
  ...
  select case( trim(particleGroup%particle_kind) )
    ...
    case('DPS_oneway')
      ...
      particleGroup%calcPressureForce => applyPressureForce_DPS 
    case default
      write(stdOutUnit,*) 'Warning mus_particles_initialize: unknown particle kind!'
      particleGroup%applyHydrodynamicForces => mus_particles_applyHydrodynamicForces_DPS
      particleGroup%moveParticles => mus_particles_move_DPS
      particleGroup%mapParticles => mus_particles_mapping_DPS
      particleGroup%transferMomentumToFluid => mus_particles_transferMomentumToFluid_DPS
  ...
  ! ------ INITIALIZE PARTICLE DEBUG LOGGER  -------- !
  call init_particle_logger( pgDebugLog, params%general%proc%rank )
  ! call init_particle_logger( DEMDebugLog, params%general%proc%rank )
  ...
  open( pgDebugLog%lu, file=pgDebugLog%lfile, status='old', position='append' )
    do i = 1, geometry%tree%global%nParts
      write( pgDebugLog%lu, *) 'proc ', i-1, ' TreeIDs: ', &
        & geometry%tree%Part_First(i), ' - ', geometry%tree%Part_Last(i)
    end do
  
    write( pgDebugLog%lu, *) "Perfectcube particles_per_side = ", perfectcube_params%particles_per_side
    write( pgDebugLog%lu, *) "Number of local elements Nfluids = ", scheme%pdf(lev)%nElems_fluid
    ! do i = 1, scheme%pdf(lev)%nElems_fluid
    !   write( pgDebugLog%lu, *) scheme%levelDesc(lev)%total(i)
    ! end do

    write( pgDebugLog%lu, *) "Number of halo elements Nhalos = ", scheme%pdf(lev)%nElems_halo
    ! do i = scheme%pdf(lev)%nElems_fluid + 1, scheme%pdf(lev)%nElems_local
    !   write( pgDebugLog%lu, *) scheme%levelDesc(lev)%total(i)
    ! end do
    
    ! write( pgDebugLog%lu, *) "--------------------------------------------------------"
    ! write( pgDebugLog%lu, *) "levelDesc%total(1) = ", scheme%levelDesc(lev)%total(1)
    ! write( pgDebugLog%lu, *) "levelDesc%total(Nelems_fluid) = ", scheme%levelDesc(lev)%total(  &
    !   & scheme%pdf(lev)%nElems_fluid )
    ! write( pgDebugLog%lu, *) "levelDesc%total(Nelems_local) = ", scheme%levelDesc(lev)%total(  &
    !   & scheme%pdf(lev)%nElems_local )
  ...
  select case ( trim(particleGroup%particle_kind) )
    case ('MEM')
      do while (iParticle <= particleGroup%particles_MEM%nvals)
        ! initialize  particle with some position and velocity
        ! * assigns procedure pointers to particle routines
        ! * sets the owner of the particle
        ! * sets existsOnProc mask which tells us which of our neighbor
        !   procs also has (part of) this particle
        ! * fills the exclusionList with elements belonging to particle
        ! * sets the prp_solid bits for these elements in the levelDescriptor
        ! * initializes exclusionListBuffer and makeFluidList to a certain size
        !   but does NOT fill them yet
        if(particleGroup%particles_MEM%nvals < 1) exit
    ...
    case('DPS','DPS_oneway','DPS_unittest')
      do while (iParticle <= particleGroup%particles_DPS%nvals)
        ...
        ! Set particle force to 0 initially
        particleGroup%particles_DPS%val(iParticle)%F(1:6) = 0.0_rk
        iParticle = iParticle + 1
      end do
...
end subroutine mus_particles_initialize_perfectcube
  ...
  elemPos = particle%exclusionList%Val(iElem)
    do iDir = 1,QQ
      do iField = 1, scheme%nFields
        ...
        stateVectorPos = ?IDX?(idx_idir, elemPos, nScalars, nSize)
        ...
        nghDir = ?NgDir?(idx_idir, scheme%layout%fStencil)
