...
use env_module,                        only : rk, stdOutUnit, LabelLen
...
subroutine map_to_bin( x, bins, N, ibin)
  ...
  if( bins(1) >= x .OR. x > bins(N) ) then
    write(stdOutUnit,*) "WARNING mapToBins: value outside range of bins"
    write(stdOutUnit,*) "x = ", x
    write(stdOutUnit,*) "bins(1) = ", bins(1)
    write(stdOutUnit,*) "bins(N) = ", bins(N)
