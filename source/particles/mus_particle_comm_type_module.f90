!> In this module the communication types and routines for particles
!! are defined
...
subroutine print_particles_comm( me )
  ...
  write(stdOutUnit,*) 'particles_communication_type procs:'

  write(stdOutUnit,'(A)', advance='no') '[ '
  write(stdOutUnit,format_string, advance='no') &
    &                           ( me%proc(iproc), iproc = 1,me%nProcs ) 
  write(stdOutUnit,'(A)') ']'

  do iproc = 1,me%nProcs
    write(stdOutUnit,*) '--- Proc ', me%proc(iproc), ' ---'
    write(stdOutUnit,*) 'Force: ', me%buf_force(iproc)%nParticles, ' particles'
    call print_particles_pIDvectorbuffer( me%buf_force(iproc) )
    write(stdOutUnit,*) 'vec: ', me%buf_vec(iproc)%nParticles, ' particles'
    call print_particles_pIDvectorbuffer( me%buf_vec(iproc) )
    write(stdOutUnit,*) 'state: ', me%buf_state(iproc)%nParticles, ' particles'
...
subroutine print_particles_pIDvectorbuffer( buff )
  ...
  do iParticle = 1, buff%nParticles
    write(stdOutUnit, '(A,I0,A)', advance='no') 'pID = ',  buff%val(iParticle)%pID, ' buff = ['
    write(stdOutUnit, '(6E10.3)', advance = 'no') ( buff%val(iParticle)%V(i), &
                                                  & i = 1,6                      )
    write(stdOutUnit, '(A)') ' ]'
  end do
...
subroutine print_particles_statebuffer( buff )
  ...
  ! -------------------------------------------!
  do iParticle = 1, buff%nParticles
    write(stdOutUnit, '(I0,A)', advance='no') buff%val(iParticle)%I(5), ' real buff = ['
    write(stdOutUnit, '(6E10.3)', advance = 'no') ( buff%val(iParticle)%V(i), &
                                                  & i = 1,12                      )
    write(stdOutUnit, '(A)') ' ]'
    write(stdOutUnit, '(I0,A)', advance='no') buff%val(iParticle)%I(5), ' int buff = ['
    write(stdOutUnit, '(6I0.5)', advance = 'no') ( buff%val(iParticle)%I(i), &
                                                  & i = 1,4                      )
    write(stdOutUnit, '(A)') ' ]'
  end do
...
!! simulation
subroutine find_particle_comm_procs( procs, scheme, geometry, myRank, dpad )
  ...
  integer :: coord(4), haloCoord(4)
  logical :: wasAdded
  ! Bounds for the Treelm coordinates, these run from 0 to 2**(level) 
  integer :: upperBound
  real(kind=rk) :: dx
  ...
  ! Number of lattice sites for the side length of the cube of
  ! elements we will check for procs
  Ndpad = ceiling(dpad / dx)
  ! write(stdOutUnit,*) 'find_particle_comm_procs: Ndpad = ', Ndpad

  firstHalo = scheme%pdf(lev)%nElems_fluid + 1
  lastHalo = scheme%pdf(lev)%nElems_local


  ! 1. Loop over halo elements 
  haloLoop: do iElem = firstHalo, lastHalo 
    haloCoord = tem_coordOfID( TreeID = scheme%levelDesc(lev)%total(iElem), &
                             & offset = TIDoffset                           ) 
    do nx = -Ndpad, Ndpad
      do ny = -Ndpad, Ndpad
        do nz = -Ndpad, Ndpad
          coord(:) = haloCoord(:) + (/ nx, ny, nz, 0/)
          ...
          if( any(coord(1:3) < 0) .OR. any(coord(1:3) > upperBound) ) then
            ...
            if( foundProc >= 0 .AND. foundProc /= myRank ) then
              ...
              call append( me       = procs,  & 
                ...
                &          wasAdded = wasAdded     )
              if(wasAdded) then
                ! Write TreeID and coordinate that led to appending of this
                ! proc to debug log
...
end subroutine find_particle_comm_procs
...
!! Hence the padding distance dpad should be set to the largest particle diameter in the 
!! simulation
subroutine find_particle_comm_procs2( procs, scheme, geometry, myRank, dpad )
  !> Dynamic array of procs which we will store the found procs in
  type(dyn_intarray_type) :: procs
  !> Scheme for access to level descriptor
  type(mus_scheme_type), intent(in) :: scheme
  ...
  integer :: coord(4), haloCoord(4)
  logical :: wasAdded
  ! Bound for the Treelm coordinates, these run from 0 to 2**(level) 
  integer :: upperBound

  ! searchBoxLims = [xmin, xmax, ymin, ymax, zmin, zmax]
  ! indicates where remote elements for a particular halo are 
  integer :: searchBoxLims(6)
  integer :: dirs(6)    ! indices of Cartesian directions in stencil

  real(kind=rk) :: dx
  ! For debugging
  real(kind=rk) :: coordX(3)
  ! Integer coordinates of the bounding box of the area that was
  ! searched for processes to communicate with.
  ...
  write(stdOutUnit,*) "Cartesian dir indices ", dirs(1:6)
  ...
  haloCoord = tem_coordOfID( TreeID = scheme%levelDesc(lev)%total(firstHalo), &
                           & offset = TIDoffset                           ) 
  boundingBox(1:2) = haloCoord(1)
  boundingBox(3:4) = haloCoord(2)
  boundingBox(5:6) = haloCoord(3)
  ! --- /FOR DEBUGGING --- !


  ! 1. Loop over halo elements 
  haloLoop: do iElem = firstHalo, lastHalo 
    ! First determine in which direction this halo has remote neighbors
    ! Remote neighbor = cannot be found on my process

    haloCoord = tem_coordOfID( TreeID = scheme%levelDesc(lev)%total(iElem), &
                             & offset = TIDoffset                           ) 

    call getSearchBoxLimits( elemCoord = haloCoord,    &
                           & elemPos   = iElem,        &
                           & scheme    = scheme,       &
                           & geometry  = geometry,     &
                           & dirs      = dirs,         &
          ...
          if( any(coord(1:3) < 0) .OR. any(coord(1:3) > upperBound) ) then
            ...
            if( foundProc >= 0 .AND. foundProc /= myRank ) then
              ...
              if(wasAdded) then
                ...
                coordX = getBaryOfCoord( coord  = haloCoord(1:3),              &
                                      & origin = geometry%tree%global%origin, &
                                      & dx     = dx                           )


                write(pgDebugLog%lu,*) "Halo treeID ", scheme%levelDesc(lev)%total(iElem)
                write(pgDebugLog%lu,'(A,3E17.9)' ) "halo bary   ", coordX(1:3)  
  ...
  coordX = getBaryOfCoord( coord = coord(1:3),                  &
                        ...
                        & dx      = dx                           )
  open(pgDebugLog%lu, file = pgDebugLog%lfile, status = 'old', position = 'append')
  write(pgDebugLog%lu,*) "find_particle_comm_procs2 boundingBox"
...
end subroutine find_particle_comm_procs2
...
!! Hence the padding distance dpad should be set to the largest particle diameter in the 
!! simulation
subroutine find_particle_comm_procs3( procs, scheme, geometry, myRank, dpad )
  !> Dynamic array of procs which we will store the found procs in
  type(dyn_intarray_type) :: procs
  !> Scheme for access to level descriptor
  type(mus_scheme_type), intent(in) :: scheme
  ...
  ! 1 Get index of prp_hasRemoteNeigh
!  iProp = findPropIndex( prp_bitpos = prp_hasRemoteNgh,     &
!                       & glob       = geometry%tree%global  )
  iProp = findPropIndex( prp_bitpos = prp_sendHalo,     &
                       & glob       = geometry%tree%global  )

  if(iProp < 1) then
    write(stdOutunit,*) "ERROR find_particle_comm_procs3, index of prp_hasRemoteNgh not found"
    call tem_abort()
  end if

  ! --- FOR DEBUGGING: look at the bounding box of elements we searched --- !
  ! Initialize boundingBox coordinates using the coordinates of the first halo
  elemPos = geometry%tree%property(iProp)%elemID(1) 

  elemCoord = tem_coordOfID( TreeID = scheme%levelDesc(lev)%total(elemPos), &
                           & offset = TIDoffset                             ) 
  ...
  ! 2 Loop over all elements with prp_hasRemoteNeigh
  do iElem = 1, geometry%tree%property(iProp)%nElems
    ! Get position of this element in total list
    elemPos = geometry%tree%property(iProp)%elemID(iElem) 
  ...
  end do ! iElem with prp_hasRemoteNeigh
  ! --- DEBUGGING --- !
  ! Convert boundingBox integer coordinates to cartesian and write to log
  coord(1) = boundingBox(1)
  coord(2) = boundingBox(3)
...
end subroutine find_particle_comm_procs3
