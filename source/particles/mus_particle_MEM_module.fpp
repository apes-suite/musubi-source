    ...
    linkLoop: do iDir = 1, stencil%QQN
      ...
      if( linkBordersSolid ) then
        ...
        pdf_val =  scheme%state(lev)%val(                                    &
          &   ?IDX?(idx_idir, fluidPos, scheme%varSys%nScalars, nSize), nNext)
...
subroutine updateSolidNodes(this, scheme, stencil)
  ...
  if ( stencil%restPosition > 0 ) then
    ...
    do iElem = 1, nParticleElems
      ...
      scheme%pdf(lev)%neigh( ?NGPOS?( zeroPos, elemPos, nSize )) &
        & = ?IDX?( zeroPos, elemPos, scheme%varSys%nScalars, nSize )
    end do 
  end if
  ...
  elemLoop: do iElem = 1, nParticleElems
    ...
    neighLoop: do iDir = 1, stencil%QQN
      ...
      scheme%pdf(lev)%neigh( ?NGPOS?( iDir, elemPos, nSize )) &
        & = ?IDX?( GetFromDir, elemPos, scheme%varSys%nScalars, nSize )
  ...
  particleElemLoop: do iElem = 1, this%exclusionList%nVals
    ...
    linkLoop: do iDir = 1, stencil%QQN
      ...
      if ( neighPos <= 0 .or. (btest(neighProp, prp_solid)) ) then
        ...
        ! scheme%pdf( lev )%neigh( ?NGPOS?( fluidDir, fluidPos, nElems )) =    &
        !   &  ?IDX?( fluidInvDir, fluidPos, scheme%varSys%nScalars, nSize )
        scheme%pdf( lev )%neigh( ?NGPOS?( fluidDir, fluidPos, nSize )) =    &
          &  ?IDX?( fluidInvDir, fluidPos, scheme%varSys%nScalars, nSize )
  ...
  particleElemLoop: do iElem = 1, this%makeFluidList%nVals
    ...
    linkLoop: do iDir = 1, stencil%QQN
      ...
      if ( neighPos <= 0 .or. (btest(neighProp, prp_solid)) ) then
        ...
        scheme%pdf( lev )%neigh( ?NGPOS?( iDir, elemPos, nSize )) =    &
                &  ?IDX?( inv_iDir, elemPos, scheme%varSys%nScalars, nSize )
        cycle 
      else
        ...
        scheme%pdf( lev )%neigh( ?NGPOS?( fluidDir, neighPos, nSize )) =    &
          &          ?IDX?( fluidDir, elemPos, scheme%varSys%nScalars, nSize )
...
!> computeCellMomentum computes the momentum (first-order moment) of
...
subroutine computeCellMomentum( elemPos, scheme, stencil, params, lev, j_phy )
  ...
  !> fluid stencil
  type(tem_stencilHeader_type), intent(in) :: stencil
  !> Parameters for access to conversion factors
  type(mus_param_type), intent(in) :: params
  !> Level of cell
  integer :: lev
  !> Output momentum vector j_phy = (jx, jy, jz). Physical units.
  real(kind=rk) :: j_phy(3)
  ! --------------------------------------------- !
  integer :: iDir, idx_idir
  integer :: nSize, nNext
  real(kind=rk) :: pdf_val
  real(kind=rk) :: cq_lat(3)    ! stencil direction in lat units
  real(kind=rk) :: j_lat(3)     ! momentum vector in lat units
  real(kind=rk) :: dx           ! mesh size
  ! --------------------------------------------- !
  nSize  = scheme%pdf( lev )%nSize
  nNext  = scheme%pdf( lev )%nNext
  dx = params%physics%dxLvl(lev)
  ...
  do iDir = 1, stencil%QQ
    ...
    j_lat = j_lat + cq_lat(1:3) * pdf_val
  end do

  j_phy = j_lat * dx**3 * params%physics%rho0 * params%physics%fac(lev)%vel

end subroutine computeCellMomentum
...
subroutine getAverageNeighborDensity(elemPos, scheme, stencil, lev, rho_lat )
  ...
  do iDir = 1, stencil%QQN
      ...
      if ( neighPos <= 0 .or. neighPos > nLocalElems  &
        & .or. (btest(neighProp, prp_solid)) ) then
        ! this link either points to a BC or a halo or another solid element 
        ! so skip it
        cycle

      else
        ! Get density of this fluid node
        elemOff = (neighPos-1)*scheme%varSys%nAuxScalars 
        rho_el_lat = scheme%auxField(lev)%val( elemOff + dens_pos ) 
        rho_lat = rho_lat + rho_el_lat
        Navg = Navg + 1
      end if
  end do ! iDir
  ...
  ! Take the average
  if( Navg > 0 ) then
    rho_lat = rho_lat / Navg
  else
    ! If there are no fluid elems to take the average from,
  ...
  do iElem = 1,nElems
    do iDir = 1,QQ
      do iField = 1, scheme%nFields
       ...
       scheme%state(lev)%val(                                          &
       & ?IDX?(idx_idir, particle%exclusionList%val(iElem), nScalars, nSize),:) &
       & = 0.000000001_rk
      end do
    end do
  end do
